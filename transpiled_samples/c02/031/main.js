...
var prefectureList = document.querySelectorAll('#prefecture-list button'); // 文字が入力される度に、内容のチェックを行う

searchWordText.addEventListener('keyup', function () {
  // 検索語の最初
  var searchWord = searchWordText.value; // 県名リストについてループ
  // elementはそれぞれの要素にあたる
  ...
  prefectureList.forEach(function (element) {
    ...
    var prefectureName = element.dataset.name; // ふりがなを取得

    var phonetic = element.dataset.phonetic; // 検索語の最初の一文字が一致するかどうかで、hideクラスの付与を決定する
