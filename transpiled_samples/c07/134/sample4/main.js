// ファイルアップロードゾーン
var fileZone = document.querySelector('.file-zone');
...
fileZone.addEventListener('drop', function (event) {
  ...
  // Fileオブジェクトを参照
  var transferdFiles = event.dataTransfer.files;
...
function displayImages(transferdFiles) {
  ...
  // ファイル数
  var fileNum = transferdFiles.length;
  ...
  // 画像表示エリアの参照
  var imagePreviewArea = document.querySelector('.image-list');

  // 各画像ファイルについて処理
  for (var _i = 0, _imageFileList = imageFileList; _i < _imageFileList.length; _i++) {
    var imageFile = _imageFileList[_i];
    // 画像ファイルの読み込み処理
